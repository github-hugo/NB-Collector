...
public class HttpsUtil extends DefaultHttpClient {
	...
	public void initSSLConfigForTwoWay() throws Exception {
		...
		Resource resource_selfcertpath = new ClassPathResource(NBConstant.SELFCERTPATH);
		KeyStore selfCert = KeyStore.getInstance("pkcs12");
		selfCert.load(resource_selfcertpath.getInputStream(), NBConstant.SELFCERTPWD.toCharArray());
		KeyManagerFactory kmf = KeyManagerFactory.getInstance("sunx509");
		kmf.init(selfCert, NBConstant.SELFCERTPWD.toCharArray());

		// 2 Import the CA certificate of the server,
		Resource resource_trustcapath = new ClassPathResource(NBConstant.TRUSTCAPATH);
		KeyStore caCert = KeyStore.getInstance("jks");
		caCert.load(resource_trustcapath.getInputStream(), NBConstant.TRUSTCAPWD.toCharArray());
	...
	public void initSSLConfigForOneWay() throws Exception {
		...
		KeyStore caCert = KeyStore.getInstance("jks");
		caCert.load(new FileInputStream(NBConstant.TRUSTCAPATH), NBConstant.TRUSTCAPWD.toCharArray());
		TrustManagerFactory tmf = TrustManagerFactory.getInstance("sunx509");
		tmf.init(caCert);
		...
		// 2 Set the domain name to not verify // (Non-commercial IoT
		// platform,no use
		// domain name access generally.)
		SSLSocketFactory ssf = new SSLSocketFactory(sc, SSLSocketFactory.ALLOW_ALL_HOSTNAME_VERIFIER);

		// If the platform has already applied for a domain name which matches
		// the
		// domain name in the certificate information, the certificate //domain
		// name
	...
	private HttpResponse executeHttpRequest(HttpUriRequest request) {
		...
		try {
			...
			logger.error("executeHttpRequest failed.");
		} finally {
			try {
